...
app.post('/events/:id/reserve', authenticateAndVerify, async (req, res) => {
 const { id } = req.params;
 const user = req.user;
 ...
 try {
   ...
   if (event.reservations.includes(user._id)) {
     return res.status(400).json({ message: 'You have already reserved this event' });
   }


   // Add the user's ID to the event's reservations array
   event.reservations.push(user._id);
   await event.save();


   res.json({ message: 'Event reserved successfully' });
 } catch (error) {
   console.error(error);
   res.status(500).json({ message: 'Failed to reserve event', error: error.message });
 }
});
